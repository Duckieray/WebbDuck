...
def test_qwen_descriptor_exposes_implemented_workflows():
...
def test_qwen_backend_selects_inpaint_when_mask_is_supplied(monkeypatch):
    captured = {}
    ...
    backend.generate(
        _descriptor(),
        {
            ...
            "input_image": Image.new("RGB", (64, 64), "white"),
            "mask_image": Image.new("L", (64, 64), 255),
            "seed": 123,
        },
    )

    assert captured["payload"]["operation"] == "inpaint"
    assert captured["payload"]["input_image"]
    assert captured["payload"]["mask_image"]
