...
- **Server/UI wiring:** `POST /generate` with `identity_adapter.type=krea2_identity_edit`; persona presets round-trip `ref_boost`/`grounding_px`/`lora_scale`/`lora_rank` via `_resolve_identity_adapter_preset`. `PersonaIdentityUI.js` switches the provider on the selected model family (krea → no silent `faceid_sdxl` fallback), remaps the strength slider to 0..10 `ref_boost` (default 4.0), reveals `.krea2-only` Grounding Res + LoRA Rank controls, and caps references at one anchor. `modelCapabilities.js` stays architecture/provider-agnostic — `section-ip-adapter` is gated only by the `identity_adapter` capability.
