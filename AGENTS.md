...
- **Megapixel default 2.0:** `DEFAULT_MAX_MEGAPIXELS` 1.0 → 2.0 so 832×1216 (~1.01 MP) and 1152×1728 (~1.99 MP) identity edits pass without the old cap downscaling them; validation clamp is still 0.125..2.0 and explicit lower per-request caps still downscale.
- Tests: resident-prefix equivalence (paired+nohooks == streaming-only math, recorded move counts per block in `test_krea2_identity.py`) + `_identity_resident_blocks` thresholds/override-env (`test_krea2_identity_runtime.py`); priority-cap tests re-based to the 2.0 default with a new `test_default_cap_allows_portrait_1152x1728`; snapshot/payload defaults updated — 89 pass total across `test_krea2_identity.py` + `test_krea2_adaptive_runtime.py` + `test_krea2_identity_runtime.py`.
