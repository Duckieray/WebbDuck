...
### Native FLUX.2 latent-init img2img (2026-08-31)
- Real FLUX.2 img2img implemented in `core/backends/flux_worker.py`: a source body + `strength < 1.0` makes the body the latent-init (VAE-encoded + noised), and only the identity references stay as image conditioning. `core/backends/flux.py` forwards `strength` (default `1.0`) in the worker payload.
- Uses `Flux2KleinPipeline.__call__(latents=..., sigmas=...)` (no `strength` arg): `_img2img_sigma_schedule(steps, strength, full_sigmas)` truncates the real dynamic-shifted sigma schedule (Klein sets `use_dynamic_shifting=true` and does NOT null passed sigmas), and `_build_img2img_init` VAE-encodes the body, computes the empirical mu, recovers the full schedule, then blends `noised = image_latents*(1-init_sigma)+noise*init_sigma`. The init latent space is the BN-normalized `(B,128,h//2,w//2)` from `_encode_vae_image`; `prepare_latents` packs it internally.
- `_build_img2img_init` runs the VAE directly (outside the pipeline offload-hook path), so after encoding it explicitly `vae.to("cpu")` + `empty_cache()` before the transformer pass (mirrors the SDXL backend) and frees the transient source/init tensors.
- **Demotion-ladder bug fixed:** `_build_img2img_init` originally ran OUTSIDE the per-tier `try`, so an OOM while re-building the init on a retry tier propagated and aborted the whole job instead of stepping down. It now lives inside the try, so img2img demotes cleanly: 832x1216 → 752x1096 → 664x976.
- **GGUF/sequential-offload guard:** `enable_sequential_cpu_offload` (accelerate) moves parameters through `"meta"`, which GGUF-quantized pack tensors cannot support (`KeyError` on quant-type). Sequential offload now only applies to non-GGUF checkpoints and falls back to model CPU offload with a clear message for GGUF.
- **Hard VRAM ceiling (measured):** on a 16 GB card the GGUF 9B path holds an ~6.3 GB non-reclaimable compute workspace floor (`empty_cache` can't free it), leaving ~10 GB. 752x1096 img2img denoise exceeds it regardless of ref count (2/3 refs OOM identically) and attention slicing is a no-op for the GGUF SDPA path. The worker therefore auto-steps 3-ref img2img down to 664x976 (0.8x) — the accepted working resolution on this hardware. 752x1096+ would need a bigger GPU or a real bf16 diffusers FLUX.2 checkpoint (lower floor, enables sequential offload).
- Covered by `tests/test_flux_memory_metadata.py` (`_img2img_sigma_schedule`; all 11 pass) alongside the existing `test_flux_identity_prep.py` / `test_flux_identity_personas.py` (30 total green).

### fp8 single-file 9B now runs on 16 GB via transformer GROUP offload (2026-08-31)
- **The earlier "hardware ceiling" was wrong.** `pipe.enable_model_cpu_offload()` on `Flux2KleinPipeline` is NOT sufficient for the 9B: accelerate's device hooks pull the ENTIRE bf16 transformer (~11.4 GB) onto the GPU the instant the transformer forward starts, leaving ~0.15 GB free — the first LayerNorm allocation OOMs even at 768x768, and the demotion ladder can't help because the transformer stays resident across retries (the retry loop only calls `empty_cache()`, which does nothing for live weight tensors).
- **Group offload is the fix.** `pipe.transformer.enable_group_offload(onload_device="cuda", offload_device="cpu", offload_type="block_level", num_blocks_per_group=1, use_stream=False, low_cpu_mem_usage=True)` streams transformer blocks from CPU RAM, keeping only ONE block resident at a time. Measured: bf16 transformer total = 16.91 GB (fp8 file upcast to bf16 for compute — fp8 CUDA matmul is `NotImplementedError` on torch 2.12.1+cu130), but only **~0.19 GB GPU-resident** during the 1024x1024 forward. Generates in ~58 s on the 16 GB RTX 5070 Ti.
- **VAE must stay on GPU with group offload.** The full-pipeline offload hooks are NOT installed in the group-offload path, so the tiny (~0.16 GB) VAE must be placed on `device` explicitly — otherwise `vae.decode` fails with `Input type (CUDABFloat16Type) and weight type (CPUBFloat16Type)`. VAE tiling/slicing are still applied as cheap decode safeguards; the Qwen3 text encoder stays on CPU (phased encoding).
- Wired in `core/backends/flux_worker.py` offload selection: `use_group_offload` is chosen when CPU offload would otherwise be selected (`use_cpu_offload`) AND `model_format != "gguf"` (GGUF in-place packed tensors can't move through device hooks — unchanged, falls back to model CPU offload). GGUF/diffusers-resident/sequential paths are untouched.
- Verified end-to-end through the server (`/generate` for `snofsSexNudesAndOther_v14Klein9bDistilled`, 1024x1024, job `completed`, real PNG). Tests: `test_flux_memory_metadata.py` + `test_flux2_single_file.py` + `test_flux_backend.py` = 31 pass; the one failure (`Flux2KleinPipeline` import in the base env) is the pre-existing env-drift, not this change.

### LoRA trigger auto-detection & 5 new Flux2 LoRA metadata entries (2026-09-02)
- **New schema field:** `_LoraMeta.trigger: str` added to `models/metastore.py` (parse/save/merge/`update_lora_meta` all handle it). `_merge_lora` prefers the metastore `trigger` over the registry's, with registry fallback — so hand-edited richer metastore triggers win.
- **Auto-detection:** `detect_lora_trigger(lora_path)` in `models/registry.py` reads only the safetensors `__metadata__` header (never tensors). Source priority: explicit `ss_trigger_word`/`trigger_word`/`trigger` → `ss_tag_frequency` JSON (highest-count non-empty inner token) → `notes`/`ss_notes`/`ss_training_comment` via a `Trigger word:` regex (single token, strips trailing `.,-`).
- **No-overwrite guarantee:** `ensure_lora_registry()` and `sync_lora_registry_file()` only fill unset/`None` triggers; manually-set triggers (e.g. `braces, braces on teeth`, `fluxtrait, detailed skin`) are never overwritten.
- **Metastore mirror:** `sync_lora_meta_triggers()` in `registry.py` copies registry triggers into the metastore only when the metastore entry has none; wired into `refresh_registries()` and module-bottom init. Lazy-imports `models.metastore` inside the function (registry loads first in `models/__init__.py`, so no circular-import break).
- **5 new Flux2 LoRA entries** added to both the registry `loras.json` and `webbduck_meta/loras.json` (key = safetensors file stem): SexGod Masturbation (`MASTURBATE`, 0.9), Lingerie (`l1ngerie`, 0.8), Braces (`braces, braces on teeth`, 0.9), Ruined makeup (`ru1ned, she has runny makeup.`, 0.7), Excessive cum/bukkake (`semen, cum`, 0.9). Trigger backfills: `motion_stabilizer_ltx_lora_v2`→`mvmt`, `SEXGOD_PregnantNudity_Klein9b_v1`→`SEXGOD`.
- **Note:** CivitAI HTML pages (civitai.red + API) return 403 via webfetch; use `https://civitai.red/api/v1/models/<id>` JSON instead. The 6th LoRA (`Cum Across Anywhere` → `Cum_on_Face.safetensors`) is NOT on disk, so it got no entry.
- Tests: new `tests/test_lora_trigger_detection.py` (6) + `tests/test_metastore.py` (32) = 38 pass. In this dev env `LORA_ROOT` resolves to the repo's empty `lora/`, so the real registry/meta backfills were applied explicitly.

### Krea Identity Edit end-to-end (feature/krea2-identity-edit, 2026-09-08)
- Delivered in phases and pushed on branch `feature/krea2-identity-edit` (`957330f` → `cb95c03`): identity adapter contract + Krea2Edit helpers, backend identity payload threading, krea-aware persona UI, the phased GPU identity runtime with FP8 LoRA residuals, identity-aware adaptive planning + reusable OOM reconfigure, and capability-driven UI tests.
- **Identity runtime:** `core/backends/krea2_identity.py` is the pure-Python contract layer (validation, `IdentitySettings` snapshot, `resolve_identity_weight` for the `conradlocke/krea2-identity-edit` LoRA, `edit_target_size`/`grid_dims`/token-count helpers returning `(height, width)`). `krea2_worker.py` sequences `[text | source(frame=1) | target(frame=0)]`, keeps only target tokens, and applies the identity LoRA as an FP8-resident `y = BaseFP8(x) + coef * B(A(x))` residual via `_linear_lora_coefficient` with `coef = (alpha/rank) * lora_scale` and explicit `is not None` guards (`lora_scale=0.0` must not collapse to the `or 1.0` fallback).
- **Adaptive planning:** `krea2_worker_adaptive.py` halves the identity token budget (3584 → 1792 for the standard profile) and plans `(reference_tokens, target_tokens, combined_image_tokens)` mirroring the worker's geometry math; the plan record carries an `identity` accounting dict. Identity jobs are never steps-tuned and take no geometry action when the reference is unreadable.
- **OOM retry reconfigure:** `configure_krea_identity_transformer(pipe, *, load_info, transformer_storage_gb, reserve_gb, mode_override=None)` is the shared helper (offload → gc → `empty_cache` → re-probe → `_configure_execution`) used both at initial setup and on the resident→block-offload demotion tier.
- **Server/UI wiring:** `POST /generate` with `identity_adapter.type=krea2_identity_edit`; persona presets round-trip `ref_boost`/`grounding_px`/`lora_scale`/`lora_rank` via `_resolve_identity_adapter_preset`. `PersonaIdentityUI.js` switches the provider on the selected model family (krea → no silent `faceid_sdxl` fallback), exposes a single unified 0..1 **Identity Strength** slider for every provider (SDXL/FLUX: `adapter_scale = slider`; Krea: `ref_boost = 1 + 10*slider`, so 0.1 = the balanced 2.0 default, 1.0 = max 10), reveals `.krea2-only` Grounding Res + LoRA Rank controls, and caps references at one anchor. `modelCapabilities.js` stays architecture/provider-agnostic — `section-ip-adapter` is gated only by the `identity_adapter` capability.
- **Phase 6 tests:** `test_capability_driven_ui.py` pins the contract (12 pass); `test_ui_sanity.py` gained a `@pytest.mark.browser` krea-provider test that switches to a krea-named model and asserts provider/control adaptation (skips when no krea-named model is on disk; note `pytest-playwright` is not installed in the webbduck env — browser assertions were smoke-verified via playwright sync API directly).
- **Phase 7 hardware smoke:** `tools/run_hardware_smoke.py` gained `--readiness-timeout` (default 300) because `/runtime-readiness` takes ~105 s on the 5070 Ti dev host; `tests/test_hardware_smoke_runner.py` now covers it (5 pass) and the pre-existing `'"/runtime-readiness"'` assertion was fixed to the actual `f"{base_url}/runtime-readiness"` f-string form. Preflight against the live 8010 host: `sdxl` SKIPPED (optional, no `--sdxl-model`), `dark-beast-fp8` READY, `flux`/`krea-turbo`/`qwen-image-2512` BLOCKED (no matching public model on this host). A krea identity generation run needs an actual krea-named checkpoint; none is installed here.
- Docs synced: `AGENTS.md` log, `.agents/frontend.md` (+`modelCapabilities.js`, `PersonaIdentityUI.js`), `.agents/backend-runtime.md` (Phase 5 section), `docs/ARCHITECTURE.md` (+8 krea2 backend rows), `docs/USER_GUIDE.md` (Krea identity bullet), `docs/HARDWARE_SMOKE_MATRIX.md` (`--readiness-timeout`), `ui/README.md`, `tests/README.md`, `README.md` (identity capability line + `WEBBDUCK_KREA2_IDENTITY_WEIGHT`).

### Krea 2 identity first real GPU run + stream-offload bug + strength default (2026-09-08/09)
- **First live krea2_identity_edit generation on the 5070 Ti** (`darkBeast30BF16INT8_darkBeast330`, 768+1024, one 4865x7297 reference) fixed a chain: (1) block-level group offload leaves the unmatched top-level modules (time_embed, img_in, text_fusion, txt_in, rotary_emb, time_mod_proj, final_layer) on CPU unless pinned — added `_pin_krea_transformer_outer_modules()` right after `enable_group_offload`; (2) LoRA residual factors are CPU-stored storage-oriented and were only cast to `x.dtype` — both residual sites (`krea2_worker.py` ScaledFP8Linear, `krea2_native_fp8.py` fallback) now cast to `device=x.device` too; (3) `KeyError: pinned_memory[buffer]` — the group hooks snapshot `cpu_param_dict` at `enable_group_offload` time, so identity LoRA/processor install was moved BEFORE `configure_krea_identity_transformer`.
- **Streamed block offload accumulates.** `transformer-block-stream-1` never returns GPU block weights after our manual per-block forward: the worker shows +0.449 GiB allocated per block and OOMs at block ~9 (11.6 GiB) regardless of resolution. FLUX.2 hit the same wall; the proven fix is `use_stream=False` (sync offload). `_configure_execution` now forces sync for the block path → `transformer-block-sync-2`, runs 768x768 10-step in ~4 m 45 s and 28-step in ~11 m on 15.5 GiB. The fp8 checkpoint stores 12.2 GiB on CPU and only ~0.42 GiB/block touches the GPU.
- **ref_boost is the composition locker.** A/B on the reference prompt: `ref_boost=4.0` output correlates 0.81 with the reference (target→source +log(4) attention bias copies the whole frame, not just the face); `ref_boost=1.0` drops to 0.695 and follows the prompt. The three-knob profile: steps barely mattered (28-step 4.0 vs 10-step 4.0 = 0.948 correlated) — ref_boost dominated. Consequently `DEFAULT_REF_BOOST` (and the server preset default) moved 4.0 → 2.0, and the Krea Identity Strength slider now maps to `ref_boost = 1 + 10*slider` with slider default 0.1 (2.0).
- **Unified Identity Strength slider:** `PersonaIdentityUI.js` now exposes the same 0..1 slider for every provider — SDXL/FLUX `adapter_scale = slider`, Krea `ref_boost = 1 + 10*slider` — so switching models no longer changes the knob semantics; persona save/load maps `ref_boost` ↔ slider position. Docs (`ui/README.md`, `docs/USER_GUIDE.md`, `.agents/*`) updated to match; `tests/test_krea2_identity.py` defaults snap to 2.0.
- Debug instrumentation (env-gated `WEBBDUCK_KREA2_DEBUG_PRINT`): `_probe_mem()` stage probes in `edit_transformer_forward`/the mask-compat processor and configure-time `KREA2_DEBUG` dumps in `krea2_worker.py`/`krea2_identity.py` — prints land in the failed-job error payload.

### Krea 2 identity per-step cost diagnosis + paired streaming + A/B knobs (2026-09-09)
- **Why identity edits are slow:** each denoise step runs **two full ~30B-fp8 transformer forwards** (CFG positive + negative; `guidance > 0` always encodes the uncond prompt, and the UI cfg default 7.5 reaches the worker), with every transformer block streamed CPU→GPU→CPU per forward (`transformer-block-sync-2`, ~0.42 GiB/block, no H2D/compute overlap). 28 steps ≈ 56 forwarded streams. Low VRAM (~6 GB) is *by design* (only ~2 blocks resident), not headroom that predicts speed.
- **Paired-block mode (`WEBBDUCK_KREA2_IDENTITY_PAIRED`, default on for CUDA):** `edit_transformer_forward_paired` in `krea2_identity.py` loads each transformer block **once per step** and runs the positive *and* negative rows through it before offloading — identical math to two `edit_transformer_forward` calls (independent forwards, no attention coupling), halving the streaming/Python-dispatch churn. New execution mode string `paired-block` in `_configure_execution` (no accelerate hooks; outer modules pinned via `_pin_krea_transformer_outer_modules`). `paired_oom` retries fall back to hook-based `transformer-block`.
- **A/B performance knobs (env-only, identity path only — text2img untouched):** `WEBBDUCK_KREA2_IDENTITY_STEPS` (int), `WEBBDUCK_KREA2_IDENTITY_GUIDANCE` (float; `<= 0` skips the uncond forward entirely), `WEBBDUCK_KREA2_IDENTITY_CFG_FREE=1` (shorthand → guidance 0, single forward per step). Wiring lives in `apply_identity_perf_overrides` (`krea2_identity.py`). These make the two big levers (drop CFG for identity; cut identity step count) side-by-side-testable on hardware without code changes.
- Tests: `edit_transformer_forward_paired` == two single forwards (mock transformer equivalence, diff text lengths, maskless ref_boost, equal-row check) + `apply_identity_perf_overrides` env semantics — 7 new; `test_krea2_identity.py` + `test_capability_driven_ui.py` = 63 pass, all ≤5 s CPU.

### Krea 2 identity resident-block prefix + 2.0 megapixel default (2026-09-08)
- **Resident block prefix:** transformer weights don't change during denoise, so `_configure_execution` (paired branch) now stages the first N leading blocks on GPU once per job (stacked FP8, ~0.42 GB/block) and records the count on `pipe.transformer._krea_resident_blocks`. `edit_transformer_forward_paired`/`edit_transformer_forward_nohooks` take `n_resident` and never re-stream/offload the resident prefix — pure transfer savings per step. `_identity_resident_blocks()` budgets free VRAM minus the resolution's activation reserve minus `_KREA_RESIDENT_WORKSPACE_GB` (1.5) — floor 0, cap `n_blocks`; measured 768×768 on the 5070 Ti lands ~20 of 28 blocks resident + pinned outer modules. `WEBBDUCK_KREA2_IDENTITY_RESIDENT_BLOCKS` (auto/‑1/empty = auto, `0` disables) forces the count. Execution-mode strings: `paired-block-N` (partial) / `paired-resident` (all 28). OOM ladder: `paired_resident_oom` (drop prefix, keep paired streaming) → `paired_oom`/`resident_oom` → hook-based `transformer-block`. Runtime dict adds `resident_blocks`.
- **Megapixel default 2.0:** `DEFAULT_MAX_MEGAPIXELS` 1.0 → 2.0 (validation clamp 0.125..2.0; explicit lower caps still downscale). **The megapixel cap is NOT the binding identity limit** — the adaptive target-grid token budget is (1792 = half the text2img 3584 on this card), which collapses any identity request ≥ ~800×1200 to ~528×784 effective. 832×1216 (~1.01 MP) and 1152×1728 (~1.99 MP) cannot natively fit an identity forward: the combined `[text | ref | target]` sequence would need 3800+ / 7776+ target grid tokens.
- **Identity size knob:** `identity.token_budget` (request payload) or env `WEBBDUCK_KREA2_IDENTITY_TOKEN_BUDGET` (`auto`/empty = 1792 halved default; integer = explicit, snapped to the 16-token grid; request beats env) A/B's the target-grid budget. Hardware-validated tiers on the 5070 Ti: 1792 → 528×784 (0.41 MP, ~3800 combined+text), 2048 → 576×864 (~4240), 2688 → 672×1008 (~5650), 3584 → 768×1152 (~7320). All completed without NaN/fallback, but the model's ~4096 training envelope is crossed between 2048 and 2688 — sequence-length extrapolation may degrade identity/attention quality, so eyeball before promoting a tier.
- Tests: resident-prefix equivalence (paired+nohooks == streaming-only math, recorded move counts per block in `test_krea2_identity.py`) + `_identity_resident_blocks` thresholds/override-env (`test_krea2_identity_runtime.py`); priority-cap tests re-based to the 2.0 default with a new `test_default_cap_allows_portrait_1152x1728`; snapshot/payload/planner `token_budget` override tests (`test_krea2_identity.py`, `test_krea2_adaptive_runtime.py`) — 92 pass across `test_krea2_identity.py` + `test_krea2_adaptive_runtime.py` + `test_krea2_identity_runtime.py`.

### Krea identity output quality: token budget is NOT the lever — auto-upscale is (2026-09-08)
- **A/B conclusion:** raising `identity.token_budget` past ~2048 is a quality risk (combined `[text | ref | target]` sequence exceeds the ~4096-token LoRA training envelope). The user judged the 2048 vs default fixed-seed pair **both poor — "compressed, no detail"**. The real problem wasn't token budget: the decoded artifact was saved at the VRAM-capped effective grid (528×784 for an 832×1216 request) with **no upscale back**, so it read as a 0.4 MP image blown up to ~1 MP. The token-budget knob stays env+request-only (user: no UI wiring).
- **Auto-upscale added (server-side):** the worker saves at effective res; on adaptation the server path (`core/backends/krea2.py`, webbduck env — NOT the isolated runtime, which lacks realesrgan) applies `_maybe_upscale_identity_artifact` to each returned image: Real-ESRGAN x2/x4 (smallest factor clearing the target short edge; `WEBBDUCK_WEIGHTS_DIR`→`WEBBDUCK_MODELS_DIR/weights`→repo `weights/`) then LANCZOS to the exact `requested_width`/`requested_height`. Plain LANCZOS fallback if weights/libs missing. Original dims come from `settings["requested_*"]` (the worker's `request` carries only adapted dims — a naive worker-side upscale targets the wrong size). Disable: `WEBBDUCK_KREA2_IDENTITY_UPSCALE=0`. Recorded as `settings["krea_upscale"]` + `performance_timing.krea_upscale_seconds`.
- **Corrects a wrong assumption in the prior commit:** the megapixel cap (2.0) was never going to natively fit 832×1216 — `docs/KREA2_PERFORMANCE.md` + `.agents/backend-runtime.md` now document the token budget as the effective size limiter and upscale as the artifact fix.
- **Verified on GPU:** 832×1216 identity (same seed 20260908) → 528×784 effective → `realesrgan-x2` → **832×1216** PNG, `post: {"from":[528,784],"to":[832,1216],"upscaler":"realesrgan-x2"}`, ~2.1 s. Side-by-side raw-vs-upscaled sheet at `/tmp/opencode/identity_upscale_before_after.png` (same seed, guaranteed-compare). 97 tests pass (`test_krea2_identity*.py` + `test_krea2_adaptive_runtime.py`).
